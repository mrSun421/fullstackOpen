...
const Notification = ({ message, successfulNotification }) => {
  ...
  if (successfulNotification) {
    return <div className="successfulNotification">{message}</div>;
  } else {
...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    const currentName = persons.find(
        ...
        personService
          ...
          .then((response) => {
            ...
            setNotificationMessage(
              `changed ${response.data.name}'s phone number`
            );
            setTimeout(() => {
  ...
  const removePerson = (id, name) => {
    if (window.confirm(`are you sure you want to delete ${name}?`)) {
      personService.remove(id).then((_response) => {
        setSuccessfulNotification(true);
